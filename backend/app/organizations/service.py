...
# ---------------------------------------------------------------------------
# Import permission system (RepoMind 2.0 RBAC)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Legacy authorization helpers (kept for backward compatibility)
# New code should use assert_org_permission() directly.
# ---------------------------------------------------------------------------

def verify_org_role(db: Session, user_id: int, organization_id: int, required_role: RoleEnum = RoleEnum.DEVELOPER):
    """
    Legacy role-level check. Kept for backward compatibility.
    New routes should use assert_org_permission() with specific permissions.
    """
    ...
    user_level = ROLE_HIERARCHY.get(
        membership.role.value if hasattr(membership.role, "value") else str(membership.role), 0
    )
...
def verify_org_admin(db: Session, user_id: int, organization_id: int):
    """Legacy: require at least org_admin level."""
    assert_org_permission(db, user_id, organization_id, Permission.MEMBERS_INVITE)

def verify_org_member(db: Session, user_id: int, organization_id: int):
    """Legacy: require at least member (developer) level."""
    assert_org_permission(db, user_id, organization_id, Permission.ORG_READ)
