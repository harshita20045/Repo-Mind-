...
export default function ReviewStateIndicator({ status, isPending, error, onTrigger }) {
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center py-16 px-6 text-center animate-fade-in">
        ...
        <button
          onClick={onTrigger}
          className="px-5 py-2 bg-surfaceHighlight hover:bg-surfaceElevated text-text-primary text-sm font-medium rounded-lg border border-white/10 transition-all"
        >
          Try Again
        </button>
      </div>
  ...
  if (status === 'failed') {
    return (
      <div className="flex flex-col items-center justify-center py-16 px-6 text-center animate-fade-in">
        ...
        <button
          onClick={onTrigger}
          className="px-5 py-2 bg-danger hover:bg-danger/90 text-white text-sm font-medium rounded-lg shadow-sm shadow-danger/20 border border-white/10 transition-all"
        >
          Retry Review
        </button>
      </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-6 text-center animate-fade-in">
      <div className="w-16 h-16 rounded-2xl bg-surfaceHighlight/70 border border-white/[0.08] flex items-center justify-center text-text-muted mb-6 shadow-inner">
      ...
      <button
        onClick={onTrigger}
        disabled={isPending}
        className="inline-flex items-center gap-2.5 px-6 py-2.5 bg-primary hover:bg-primary-hover text-white text-sm font-semibold rounded-xl transition-all shadow-lg shadow-primary/20 border border-white/10 disabled:opacity-50"
      >
